...
const StakingDetails = ({ vaultId, address }) => {
  ...
  const ethersContract = new ethers.Contract(contracto, VAULTABI, provider);

  const [stkedNfts, setStaked] = useState([]);
  const [dailyE, setDailyE] = useState(0);
  ...
  useEffect(() => {
    async function data() {
      await fetch(`${API_URL}/token/${address}`)
        .then((x) => x.json())
        .then(async (d) => {
          const y = d[vaultId];
  ...
  useEffect(() => {
    //if (address != undefined)  console.log((stakedTokens))
    const x = async () => {
      let count = 0;
      stakedTokens?.map(async (token) => {
       const prom = new Promise(async(resolve, reject) => (count += parseInt(
          resolve(parseInt(await ethersContract.rewardR(vaultId, parseInt(token[1]))))
        ))).then(res => setDailyE(dailyE + ((res)/1E18)))
      });
      // setDailyE(count)
      console.log(dailyE)
    };
    ...
    <div className="stats-container mx-6 text-center">
      ...
      <div className="flex justify-between space-y-3 flex-col md:flex-row md:justify-between p-1 md:space-x-2">
        <div className="stats mt-3">
          ...
          <p className="text-xl extra text-yellow-400 font-semibold">
            {earned ? earned : 0} HACT
          </p>
        </div>
        <div className="stats">
          <h2 className="text-sm">Daily Earning</h2>
          <p className="text-xl extra text-yellow-400 font-semibold">
            {parseInt(dailyE) ? parseInt(dailyE) : 0} HACT
          </p>
        </div>
        <div className="stats">
        ...
        <div className="stats">
          ...
          <p className="text-xl extra flex flex-row justify-center items-center text-center text-yellow-400 font-semibold">
      ...
      <div className="items-center flex justify-center text-center mt-3">
        <Button onPress={callClaim} auto flat bordered color="warning">
          Claim Rewards
        </Button>
      </div>
    </div>
